...
（已压缩，保留最近 {self.config.min_retain_rounds} 轮完整对话）"""
        ...
        if tool:
            try:
                ...
                if response.status == ToolStatus.ERROR:
                    ...
                    preview = self._truncate_tool_output(tool_name, response.text)
                    return f"⚠️ 部分成功: {preview}"
                else:
                    return self._truncate_tool_output(tool_name, response.text)
    ...
    def _truncate_tool_output(self, tool_name: str, text: str) -> str:
        """Truncate tool output for sync execution."""
        if not text:
            return text

        truncate_result = self.truncator.truncate(tool_name=tool_name, output=text)
        preview = truncate_result.get("preview", text)

        if truncate_result.get("truncated"):
            full_path = truncate_result.get("full_output_path")
            if full_path:
